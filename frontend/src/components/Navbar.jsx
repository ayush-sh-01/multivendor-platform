...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-[#2C4234] text-[#F4EFE6] transition-all duration-300 border-b border-[#F4EFE6]/10">
      <div className="flex justify-between items-center w-full px-5 md:px-8 py-2.5 max-w-[1440px] mx-auto min-h-[48px]">
        {/* Left: Brand Logo in Serif with Star Icon */}
        <div className="flex items-center gap-6">
          <a
            href="#"
            className="font-serif text-lg md:text-xl font-bold tracking-tight text-[#F4EFE6] hover:text-[#BA9F7A] transition-colors flex items-center gap-1.5"
          >
            <span>ThriftCampus+</span>
            <span className="text-[#BA9F7A] text-sm select-none">✦</span>
          </a>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex items-center">
            {isSearchOpen ? (
              <div className="flex items-center bg-[#203227] border border-[#BA9F7A]/40 px-3 py-1 rounded-full transition-all">
                <input
                  type="text"
                  placeholder="Search pieces..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="bg-transparent text-xs font-sans outline-none text-[#F4EFE6] w-28 sm:w-40 placeholder-[#F4EFE6]/40 px-1"
                  autoFocus
                />
                <button
                  onClick={() => {
          ...
          >
            ...
            {totalCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#B56653] text-[#F4EFE6] text-[9px] font-bold w-3.5 h-3.5 rounded-full flex items-center justify-center">
                {totalCount}
              </span>
            )}
          </button>
